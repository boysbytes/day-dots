document.addEventListener('DOMContentLoaded', function () {
    ...
    function generateDays(daysData) {
        ...
        const allDays = []; // Array to hold all day elements
        ...
        // Adjust grid positioning for day-period elements
        const dayPeriodElements = daysContainer.querySelectorAll('.day-period');
        let currentColumn = 1;
        let rowStartDate = 0;

        dayPeriodElements.forEach((dayPeriodElement, index) => {
            const previousElement = allDays[allDays.indexOf(dayPeriodElement)-1];

            if(previousElement && previousElement.classList.contains('day-title')){
                const span = parseInt(previousElement.style.gridColumn.split(' ')[1]);
                currentColumn += span;
            }
            dayPeriodElement.style.gridColumnStart = currentColumn;
            currentColumn++;
        });
    }
